...
(function ($) {
    $.fn.slides = function (option) {
        ...
        //默认进入执行一次
        $(".loading").animate({"left":option.finish},option.play,function(){
            $(".loading").css("left","-207px");
        });
        var moveSpeed=option.loadWidth/option.play; //计算每毫米移动距离
        var x;
        return this.each(function (i) {
            ...
            if (option.hoverPause && option.play) {
                ...
                //鼠标离开
                control.children().bind('mouseout', function () {
                    //当鼠标离开后，过8000毫秒再执行
                    var timer=null;
                    if(i===0){
                        option.remainTime= $(".loading").css("left").replace(/(-|px)+/g,"");
                        option.remainTime=parseInt(option.remainTime/moveSpeed);    //根据平均速度计算剩余距离所需时间
                        //利于平均时间跑完剩余的进度，回调animate，利用surplus标记不执行进度条，直接切换下一张
                        //修改surplus标记，执行计时器，在计时器中默认再跑一遍。
                        $(".loading").animate({"left":option.finish},option.remainTime,function(){
                            $(".loading").css("left","-207px");
                            option.surplus=true;
                            animate('next', effect);
            ...
            function animate(direction, effect, clicked,flag) {
                if (!active && loaded) {
                    active = true;
                    switch (direction) {
                        case'next':
    ...
    $.fn.slides.option = {
        ...
        autoHeightSpeed: 350,
        bigTarget: false,
        //pageClass:'.bx_banner_slide .slides_container .slides_control .page',
