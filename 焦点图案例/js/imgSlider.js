...
var img = {
    ...
    //新增，取得每个li的a标签
    small_title:document.getElementById("imagebg").getElementsByTagName("a"),
    ...
    init:function(){
        ...
        for(var i=0;i<img.imagebg_li.length;i++){
            ...
            if(i!=0){
                //里面的<span class='samllImgTitle'>"+img.small_title[i].title+"</span>,是新增的。取得每个a标签里的title属性里的文字
                img.small_pic_html.push("<li onclick='img.play("+i+")'><img src='"+img.imagebg_li[i].getAttribute("data-sPic")+"' /><span class='samllImgTitle'>"+img.small_title[i].title+"</span></li>");
                img.imagebg.getElementsByTagName("li")[i].style.display ="none";
            }else{
                //里面的<span class='samllImgTitle'>"+img.small_title[i].title+"</span>,是新增的。给第一张缩略图添加文字
                img.small_pic_html.push("<li onclick='img.play("+i+")' class='currently'><img src='"+img.imagebg_li[i].getAttribute("data-sPic")+"' /><span class='samllImgTitle'>"+img.small_title[i].title+"</span></li>");
            }
